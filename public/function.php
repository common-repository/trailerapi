...
function LTM_install() {
	...
	$ltm_trailer_sql = "CREATE TABLE $ltm_trailer (
						...
						movie_imdb varchar(5)  NOT NULL,
						movie_cast text  NOT NULL,
						movie_producer text  NOT NULL,
...
function LTM_installed() {
	global $wpdb;
	$ltm_options_count_array = $wpdb->get_results("SHOW TABLES LIKE '%{$wpdb->prefix}ltm_options%'", ARRAY_A);
	$ltm_trailer_count_array = $wpdb->get_results("SHOW TABLES LIKE '%{$wpdb->prefix}ltm_trailer%'", ARRAY_A);
    if ( empty($ltm_options_count_array[0]) OR empty($ltm_trailer_count_array[0])) {
      	delete_option('LTM_version');
        LTM_install();
    } else {
		if($ltm_options_count_array[0] == ''){
			$GLOBALS['optionserror'] = $wpdb->prefix."ltm_options- Database Table  Exits";
		}
		if($ltm_trailer_count_array[0] == ''){
			$GLOBALS['optionserror'] = $wpdb->prefix."ltm_trailer- Database Table  Exits";
		}
	}
}
...
function LMT_Menu() {
	...
	add_submenu_page( 'LMT_admin', $GLOBALS['language']['check'], $GLOBALS['language']['check'], 'manage_options', 'LMT_check', 'LMT_check');
...
global $wpdb;
 ...
 $language_req='language_'.$user_options[0]['language'].'.php';
	...
	$api_url = "http://www.trailerapi.com/api/check.php";
	...
	if(!$xml = simplexml_load_string($api_url_content)) {
		echo 'XML Error, Please refresh!';
	} else {
	
	
	foreach( $xml as $film ) {
	$sql_add_array[]="post_content LIKE '%".$film->did."%'";
	}
	 $sql_post="UPDATE ".$wp_post." SET post_content = REPLACE (post_content,'//www.dailymotion.com/embed/video/','//www.trailerapi.com/api/embed.php?did=') WHERE post_status='publish' AND (".implode(" OR ", $sql_add_array).")";

	$sql=$wpdb->query($sql_post);
	echo '<div class="trailer normal caption"><div class="description"><legend><span class="number">'.$sql.'</span> '.$GLOBALS['language']['checkupdate'].'</legend></div></div>';
...
$meta_keys = $wpdb->get_results("SELECT DISTINCT (meta_key) as meta_key FROM $wpdb->postmeta WHERE meta_key NOT LIKE '%oembed%' ", ARRAY_A );
...
<?php echo $GLOBALS['language']['responsivecodewh']; ?>
<br/>
...
$ltm_trailer = $wpdb->prefix . 'ltm_trailer';
$query = $wpdb->get_results("SELECT * FROM $ltm_trailer WHERE movie_status='0' or  movie_status='4' ORDER BY movie_year DESC  Limit 20", ARRAY_A);
	$movie_list_id=1;
	?>
	<?php 
